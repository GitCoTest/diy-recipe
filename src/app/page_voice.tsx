...
export default function Home() {
  ...
  const processVoiceRequest = async (transcript: string) => {
    ...
    // Extract ingredients
    const ingredientKeywords = ['peanut butter', 'chocolate', 'egg', 'flour', 'pasta', 'chicken', 'rice', 'tomato', 'cheese'];
    ingredientKeywords.forEach(ingredient => {
      if (request.includes(ingredient)) {
        extractedData.ingredients.push(ingredient);
      }
    });

    // Extract meal type
    if (request.includes('breakfast')) extractedData.mealType = 'breakfast';
    else if (request.includes('lunch')) extractedData.mealType = 'lunch';
    else if (request.includes('dinner')) extractedData.mealType = 'dinner';
    else if (request.includes('snack')) extractedData.mealType = 'snack';
    else if (request.includes('dessert') || request.includes('cake')) extractedData.mealType = 'dessert';

    // Extract dietary preferences
    if (request.includes('vegetarian') || request.includes('eggless')) extractedData.dietary = 'vegetarian';
    else if (request.includes('vegan')) extractedData.dietary = 'vegan';
    else if (request.includes('gluten free')) extractedData.dietary = 'gluten-free';

    // Extract time constraints
    if (request.includes('quick') || request.includes('fast') || request.includes('minutes')) {
      extractedData.timeConstraint = 'quick';
    }

    // Extract cooking method
    if (request.includes('mug cake') || request.includes('microwave')) {
      extractedData.cookingMethod = 'microwave';
    }

    // Call recipe generation with extracted data
    await generateRecipesFromVoice(extractedData, transcript);
  };

  const generateRecipesFromVoice = async (extractedData: any, originalRequest: string) => {
    ...
    try {
      const response = await fetch('/api/working', {
        ...
        body: JSON.stringify({
          baseIngredients: extractedData.ingredients.slice(0, 3), // Some as base
          mainIngredients: extractedData.ingredients.slice(3), // Rest as main
          mealType: extractedData.mealType,
          dietary: extractedData.dietary,
          customizations: {
            timeConstraint: extractedData.timeConstraint,
            cookingMethod: extractedData.cookingMethod,
            voiceRequest: originalRequest
          },
          surpriseMode: extractedData.ingredients.length === 0,
        }),
      });
